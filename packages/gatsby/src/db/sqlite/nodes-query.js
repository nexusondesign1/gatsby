...
const { getdb, isArrayField, getKnownFields } = require("./nodes.js")
  ...
  switch (op) {
    case `eq`:
      ...
      if (isArrayField(path)) {
        ...
        return translatePathToWhereClause(path, 'in', [processedValue], [originalValue], preparedArgs)
      }
    ...
    case `ne`:
      ...
      if (isArrayField(path)) {
        ...
        return translatePathToWhereClause(path, 'nin', [processedValue], [originalValue], preparedArgs)
      }
    ...
    case `in`: {
      ...
      if (isArrayField(path)) {
        ...
        getKnownFields().forEach(f => {
          if (r.test(f)) {
            if (!firstField && f.endsWith(Λ + "0")) firstField = f
    ...
    case `nin`: {
      ...
      if (isArrayField(path)) {
        ...
        getKnownFields().forEach(f => {
          if (r.test(f)) {
            if (f.endsWith(Λ + "0")) firstField = f
            // You cannot bind an array so we'll have to manually scrub this in
            preparedArgs.push(...processedValue)
