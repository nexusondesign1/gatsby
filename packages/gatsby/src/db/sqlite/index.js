...
function encodeJsToSqlite(v) {
  ...
  switch (typeof v) {
    ...
    case "number":
      if (!isFinite(v)) {
        if (isNaN(v)) {
          throw new Error('sqlite/nodes.js: Error: Unable to encode NaN. (Note: an occurrence of NaN is likely to be a bug somewhere)')
        }
        throw new Error('sqlite/nodes.js: Error: Unable to encode Infinity. (Note: there is potentially a bug unless you expected this value to appear)')
      }
    ...
    case "object":
      ...
      throw new Error('sqlite/nodes.js: Error: Should only receive primitives in this function')
    default:
...
function decodeSqliteToJs(v) {
  ...
  switch (v[0]) {
    case '"': return v.slice(1) // The string is NOT encoded, only prefixed
    case 't': return true
    case 'f': return false
    case 'n': return null
    case 'u': return undefined // Shouldn't happen
    case '0':
    case '1':
    case '2':
    case '3':
    ...
    case '.': // `.5` is a valid float. sqlite may never do this tho..?
      ...
      return parseFloat(v)
    default:
      throw new Error('sqlite/nodes.js: Error: Unable to decode value, did not recognize encoding: `' + JSON.stringify(v) + '`.')
  }
}
